    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "global": {"scrape_interval": "15s", "scrape_timeout": "10s", "evaluation_interval": "30s"},
                        "scrape_configs": [
                            {
                                "job_name": "prometheus",
                                "scrape_interval": "15s",
...
async def get_config(request: Request, response: (Response or PlainTextResponse)):
...
@router.put(
    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {
                    "example": [{"status": "success", "message": "Configuration updated successfully"}]
                }
            },
        },
        400: {
            ...
            "content": {
                "application/json": {
                    "example": [
                        {"status": "error", "message": "Additional properties are not allowed (globals was unexpected)"}
                    ]
                }
            },
        },
...
async def update_config(
        ...
        Body(
            openapi_examples={
                "Update Prometheus configuration": {
                    "description": "Update entire Prometheus configuration file",
                    "value": {
                        "global": {"scrape_interval": "30s", "scrape_timeout": "30s", "evaluation_interval": "1m"},
                        "scrape_configs": [
                            {
                                "job_name": "prometheus",
                                "metrics_path": "/metrics",
                                ...
                                "static_configs": [
                                    {
                                        ...
                                        "labels": {"instance": "prometheus-server", "env:": "production"},
                                    }
                                ],
                            }
                        ],
    ...
    validation_status, response.status_code, sts, msg = validate_schema("configs.json", user_data)
    if validation_status:
        data_yaml = yaml.dump(user_data, Dumper=yaml.SafeDumper, sort_keys=True)
            ...
            openapi_examples={
                "Update Alertmanager configuration": {
                    ...
                    "value": {
                        "alerting": {
                            "alertmanagers": [
                                {"scheme": "https", "static_configs": [{"targets": ["example-alertmanager.com"]}]}
                            ]
                        }
                    },
                }
