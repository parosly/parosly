...
@router.get(
    "/metrics-lifecycle-policies/{name}",
    ...
    responses={
        ...
        404: {
            ...
            "content": {"application/json": {"example": [{"status": "error", "message": "Policy not found"}]}},
        },
    },
)
async def get_policy(
    ...
    resp = {
        200: {"status": "success", "message": "Successfully returned the requested metric lifecycle policy"},
        404: {"status": "error", "message": "Policy not found"},
    }
...
async def get_policies(
    ...
    logger.info(
        ...
        extra={"status": response.status_code, "method": request.method, "request_path": request.url.path},
    )
    return policies
    ...
    responses={
        ...
        400: {
            ...
            "content": {
                "application/json": {
                    "example": [
                        {"status": "error", "message": "Additional properties are not allowed (time was unexpected)"}
                    ]
                }
            },
        },
        409: {
            ...
            "content": {
                "application/json": {"example": [{"status": "error", "message": "The requested policy already exists"}]}
            },
        },
        500: {
            ...
            "content": {
                "application/json": {
                    "example": [{"status": "error", "message": "Failed to create metric lifecycle policy"}]
                }
            },
        },
    },
...
async def create(
    ...
    if validation_status:
        validation_status, response.status_code, sts, msg = validate_schema("policies_create.json", data)
        if validation_status:
            validation_status, response.status_code, sts, msg, val = mlp.validate_duration(data.get("keep_for"))
            if validation_status:
                response.status_code, sts, msg = mlp.create_policy(**policy.dict(), data=policies)
    logger.info(
        msg=msg,
        extra={
            "status": response.status_code,
    ...
    return {"status": sts, "policy_name": policy.dict().get("name"), "message": msg}
...
@router.patch(
    ...
    responses={
        200: {
            "description": "OK",
            "content": {
                "application/json": {"example": [{"status": "success", "message": "Policy updated successfully"}]}
            },
        },
        400: {
            "description": "Bad Request",
            "content": {
                "application/json": {
                    "example": [
                        {"status": "error", "message": "Additional properties are not allowed ('name' was unexpected)"}
                    ]
                }
            },
        },
...
async def update(
    ...
    validation_status, response.status_code, sts, msg = mlp.validate_prom_admin_api()
    if validation_status:
        validation_status, response.status_code, sts, msg = validate_schema("policies_update.json", data)
        if validation_status:
            validation_status, response.status_code, sts, msg, val = mlp.validate_duration(data.get("keep_for"))
            if validation_status:
                response.status_code, sts, msg = mlp.update_policy(name=name, data=data, policies=policies)
    logger.info(
        msg=msg,
        extra={
            "status": response.status_code,
    ...
    return {"status": sts, "policy_name": name, "message": msg}
...
@router.delete(
    ...
    responses={
        ...
        500: {
            ...
            "content": {
                "application/json": {
                    "example": [{"status": "error", "message": "Failed to delete metrics lifecycle policy"}]
                }
            },
        },
    },
)
async def delete(name: str, request: Request, response: Response):
    response.status_code, sts, msg = mlp.delete_policy(name=name, policies=policies)
    logger.info(
        msg=msg, extra={"status": response.status_code, "method": request.method, "request_path": request.url.path}
    )
    return {"status": sts, "message": msg} if response.status_code != 204 else response.status_code


@router.post(
    "/metrics-lifecycle-policies/trigger",
    ...
    responses={
        202: {
            ...
            "content": {
                "application/json": {
                    "example": [{"status": "success", "message": "Your request has been accepted for processing"}]
                }
            },
        },
        409: {
