        ...
        200: {
            ...
            "content": {
                "text/csv; charset=utf-8": {
                    ...
                    "up,parosly:5000,parosly,1719131438.585,1\n"
...
async def export(
    ...
    custom_fields, timestamp_format = data.get("replace_fields"), data.get("timestamp_format")
    validation_status, response.status_code, sts, msg = validate_schema("export.json", data)
    if validation_status:
        range_query = True if all([start, end, step]) else False
        resp_status, response.status_code, resp_data = exp.prom_query(
            range_query=range_query, query=expr, start=start, end=end, step=step
        )
        if resp_status:
    ...
    if sts == "success":
        return FileResponse(path=file, background=BackgroundTask(exp.cleanup_files, file))
