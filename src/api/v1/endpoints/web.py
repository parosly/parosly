...
if arg_parser().get("web.enable_ui") == "true":
    ...
    @router.get(
        "/", response_class=HTMLResponse, description="Renders home page of this application", include_in_schema=False
    )
    async def homepage():
        return FileResponse("ui/pages/homepage/index.html")

    @router.get("/images/{path}", description="Returns common image resources for web UI", include_in_schema=False)
    async def images(path, request: Request):
        ...
        logger.info(msg=msg, extra={"status": sts, "method": request.method, "request_path": request.url.path})
        return f"{sts} {msg}"
    ...
    @router.get("/reports", description="Renders Reports HTML page of this application", include_in_schema=False)
    async def reports_page():
        return FileResponse(f"{reports}/index.html")

    @router.get(
        "/reports/{path}", description="Returns JavaScript and CSS files of the Reports", include_in_schema=False
    )
