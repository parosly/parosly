...
async def _reverse_proxy(request: Request):
    ...
    rp_req = client.build_request(
        request.method, url, timeout=None, headers=request.headers.raw, content=await request.body()
    )
    rp_resp = await client.send(rp_req, stream=True)
    logger.info(
        msg="-", extra={"status": rp_resp.status_code, "method": request.method, "request_path": rp_resp.url.path}
    )
