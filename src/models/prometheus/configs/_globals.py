"""Global Configuration model for Prometheus."""

from pydantic import BaseModel, Field


class GlobalConfig(BaseModel):
    scrape_interval: str | None = Field("1m", description="How frequently to scrape targets by default.")
    scrape_timeout: str | None = Field("10s", description="How long until a scrape request times out.")
    ...
    evaluation_interval: str | None = Field("1m", description="How frequently to evaluate rules.")
    rule_query_offset: str | None = Field(
        "0s",
    ...
    external_labels: dict[str, str] | None = Field(
        None,
    ...
    query_log_file: str | None = Field(
        None, description="File to which PromQL queries are logged. Reloading the configuration will reopen the file."
    )
    ...
    label_value_length_limit: int | None = Field(
        0,
        description="Per-scrape limit on length of labels value that will be accepted for a sample. If a label "
    ...
    target_limit: int | None = Field(
        0,
        description="Per-scrape config limit on number of unique targets that will be accepted. If more than this "
    ...
    keep_dropped_targets: int | None = Field(
        0,
        description="Limit per scrape config on the number of targets dropped by relabeling that will be kept in "
