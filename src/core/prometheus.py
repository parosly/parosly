...
class PrometheusAPIClient:
    ...
    def get_config(self) -> tuple[bool, int, dict]:
        ...
        """
        try:
            r = requests.request(method="GET", url=f"{self.prom_addr}/api/v1/status/config")
        except BaseException as e:
            return False, 500, {"status": "error", "error": f"Failed to connect to Prometheus. {e}"}
        else:
    ...
    def update_config(self, data: str) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.error(f"Failed to update Prometheus configuration file. {e}")
            return False, str(e)
        else:
            logger.debug(f"Successfully updated Prometheus configuration file: {self.prom_config_file}")
            return True, "success"
    ...
    def create_rule(self, rule: Rule, file: str = None) -> tuple[int, dict]:
        ...
        def __filename_generator() -> str:
            ...
            """
            nonlocal file
            file_prefix = f"{args.get('file.prefix')}-" if args.get("file.prefix") else ""
            file_suffix = args.get("file.extension")
            return file if file else f"{file_prefix}{uuid4()!s}{file_suffix}"
        ...
        while True:
            validation_status, status_code, status_msg, msg = validate_schema("rules.json", rule.data)
            if not validation_status:
                status_code = 400
                break
            create_rule_status, status_msg, msg = __create_rule_file(data=rule.data)
