...
def sync_to_file(data) -> None:
    ...
    logger.debug(f"Policy has been updated in file {rule_path}/{policies_data_file}.")


def create_policy(name: str, match: str, keep_for: str, description: str, data: dict) -> tuple[int, str, str]:
    """This function creates a new metrics lifecycle policy"""
    global rule_path, policies_data_file
    if name in data:
        return 409, "error", "The requested policy already exists"
    try:
        data[name] = {"match": match, "keep_for": keep_for, "description": description}
        sync_to_file(data)
...
def update_policy(name: str, data: dict, policies: dict) -> tuple[int, str, str]:
...
def load_policies() -> dict:
    ...
    policies = dict()
    try:
        with open(f"{rule_path}/{policies_data_file}") as f:
...
def validate_duration(val) -> tuple[bool, int, str, str, int]:
    ...
    """
    prom_storage_retention_human = prom_info(prom_addr, "/runtimeinfo")["data"]["storageRetention"]
    ...
    if val_seconds >= prom_storage_retention_seconds:
        return (
            ...
            422,
            "error",
            f"Invalid duration: 'keep_for' must be less than Prometheus "
            f"TSDB storage retention time, which is {prom_storage_retention_human}",
            0,
        )
...
def validate_prom_admin_api() -> tuple[bool, int, str, str]:
    prom_admin_api_status = prom_info(prom_addr, "/flags")["data"]["web.enable-admin-api"]
    if prom_admin_api_status == "false":
        return (
            False,
            500,
            "error",
            "Metrics lifecycle policy API requires enabling Prometheus admin APIs. "
