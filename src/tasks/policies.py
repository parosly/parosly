...
def delete_series(policy_name: str, policy: dict) -> bool:
    ...
    try:
        r = requests.post(f"{prom_addr}/api/v1/admin/tsdb/delete_series?match[]={policy['match']}&end={time_range}")
    except BaseException as e:
        logger.error(e, extra={"policy_name": policy_name})
    else:
        if r.status_code == 204:
            logger.debug(
                "Task clean-up time-series has been successfully completed", extra={"policy_name": policy_name}
            )
            return True
        logger.error(
            f"Failed to delete series, {r.json().get('error')}",
            extra={"status": r.status_code, "policy_name": policy_name},
        )
    return False
...
def clean_tombstones() -> bool:
    ...
    """
    try:
        r = requests.post(f"{prom_addr}/api/v1/admin/tsdb/clean_tombstones")
    except BaseException as e:
        logger.error(e)
    else:
        if r.status_code == 204:
            return True
        logger.error(f"Failed to clean tombstones, {r.json().get('error')}", extra={"status": r.status_code})
    return False
...
def run_policies() -> bool:
    ...
    if running_tasks:
        logger.warning("Cannot create a new task. Server is currently processing another task")
        return False
    ...
    if policies:
        ...
        for p in policies:
            logger.debug(
                "Task clean-up series is in progress",
                extra={"policy_name": p, "match": policies[p]["match"], "keep_for": policies[p]["keep_for"]},
            )
            delete_series(policy_name=p, policy=policies[p])
        clean_tombstones()
        exec_time = float(f"{time() - start_time:.2f}")
        running_tasks = False
        logger.debug("Task clean-up series has been completed", extra={"duration": exec_time})
    return True
